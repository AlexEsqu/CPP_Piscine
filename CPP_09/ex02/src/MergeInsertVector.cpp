...
*/
void	PmergeMe::MergeInsertSort(std::vector<int>& intVector)
{
	if (intVector.size() <= 1)
		return;
	...
	result.insert(result.begin(), small[0].value);
	...
	for (std::vector<pend>::iterator it = small.begin() + 1; it != small.end(); it++) {
		std::vector<int>::iterator bigger = std::find(result.begin(), result.end(), it->smaller_than);
		size_t pos = std::distance(result.begin(), bigger);
		binaryInsert(result, it->value, pos);
	}
